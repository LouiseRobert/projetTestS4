...
public class Arete{
	...
	public double longueur(){ // Renvoie la longueur d'une arete 
	// La longueur entre 2 Point est la racine (de la somme (des carres (des coordonees de B - celle de A)))
	...
	public void deplacer(float x, float y, float z) {// Deplace une arete dans le plan par un vecteur donnee en parametre
		// On ajoute les coordonnees et celle du vecteur de translation
	...
	public Point milieu(){// Renvoie le point du milieu de l'arete 
		float[] cA= this.coordA();// On skoque les coordonees du point A
		float[] cB= this.coordB();// On skoque les coordonees du point B
		float[] cC={0,0,0};// Initialisation des coordonnee du milieu
		// On fait la moyenne des coordonne des deux extremite de l'arete
		for (int i=0; i<3; i++){
			cC[i]=(cA[i]+cB[i])/2;
		}
